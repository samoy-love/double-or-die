 ...
 * пусто.
 *
 * Кадр здесь не читается пикселями, и это вынужденно: контекст создаётся без
 * `preserveDrawingBuffer`, и его содержимое сразу после показа не определено —
 * `readPixels` снаружи кадра возвращает то верный снимок, то пустой даже на
 * исправной игре. Пиксели проверяются там, где отрисовка синхронна и
 * управляема, — в бенче рендера по dev-сборке (`perf.spec.ts`).
 *
