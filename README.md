...
возвращает только кон, а дожатое пари платит вдвое. Забег начинается с 30
фишек: кон — это `min(тир, кошелёк)`, и с пустым кошельком вся механика
работала бы вхолостую. Размер кона задают три тира аппетита, и выбор
защёлкивается на всю комнату: отпущенная кнопка его не отменяет. Пари,
невыполнимое на схеме ввода игрока, ему просто не выпадает — исключения живут
в каталоге, а схема едет битами кадра ввода. Схем управления две: геймпад и
клавиатура с мышью, и всё, что делается с геймпада, делается с клавиатуры.

Туз выходит на арену ради подброса один раз за комнату, комментирует бой,
телеграфирует подброс, роняет карту — и через три секунды уходит: цилиндр,
зависший у стены до конца боя, перестаёт быть событием. Второй выход он тратит
«на настроение»: событие, достойное реакции, приводит его само, с тем же
телеграфом. Зовут его аплодисменты провалу, палец вниз и овация; зевок,
отворот и суета — фон, который виден, только если он и так рядом. Выходов за
комнату не больше двух, пауза между ними считается от ухода. Встаёт он в
точке, посчитанной от центра масс всех живых игроков.
Экран расчёта показывает near-miss, снятый в момент срыва пари, а не
восстановленный потом по логу.
