...
Сборка живёт на **[die.samoy.love](https://die.samoy.love)** — туда уезжает
каждый коммит в `main`. Сейчас это внутренняя альфа: публичный выпуск начинается с версии 0.5.0.
...
Версия **0.4.0 «Забег»**: полный цикл от меню до итогов. Этаж из восьми комнат,
выбор из трёх дверей с аппетитом на комнату, лавка и Дар, доля заведения с
торгом при нехватке, Ставка Крупье и экран итогов. Ставки при этом по-прежнему
живут внутри боя: карта пари подбирается на арене, «Забрать» обналичивает её
досрочно, а дожатая до конца платит вдвое.

Игра объясняет себя сама: подсказка в бою называет кнопку ровно тогда, когда
она нужна, и исчезает, как только игрок ею воспользовался. Настройки, справка
и пауза доступны прямо из забега.

Подробности механики — в [`docs/ECONOMY.md`](docs/ECONOMY.md) и
[`docs/GDD.md`](docs/GDD.md).

Дальше — 0.5.0 «Диван»: кооп на четверых и первый публичный выпуск.
