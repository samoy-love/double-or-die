 ...
 *
 * Перенести частицы «куда положено» — рассмотрено и отклонено. В `sim` им
 * нельзя по построению: частица ни на что не влияет, а в состоянии раздула бы
 * снимок, попала бы в хеш и превратила косметическую правку в слом
 * детерминизма (см. шапку `packages/client/src/particles.ts`). В `shared` —
 * тоже нет: там типы, схемы и протокол, которые читает будущий сервер, и пул
 * Float32Array со временем в секундах там инородное тело; вдобавок он тянет за
 * собой `palette.ts`, то есть в общий пакет уехал бы кусок рендера. В `tools`
 * перенос разворачивает стрелку в обратную сторону — клиент начал бы
 * импортировать инструменты, а это нарушение куда хуже нынешнего.
 * Исключение остаётся. Оно выписано на один файл, а не на пакет `tools`
 * целиком: любой второй инструмент, потянувшийся к клиенту, валит гейт и
 * приходит сюда за отдельной строкой и отдельным обоснованием.
 */
...
function imports(code: string): { spec: string; line: number }[] {
  ...
  for (const re of SPEC_PATTERNS) {
    ...
    for (const m of code.matchAll(re)) {
      ...
      const key = `${m[2]}\0${line}`;
      if (!found.has(key)) found.set(key, line);
    }
  }
  return [...found.keys()].map((k) => ({ spec: k.split('\0')[0], line: found.get(k)! }));
}
