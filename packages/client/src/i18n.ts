...
export function charset(): string {
  ...
  /*
   * Пробел остаётся в наборе, хотя рисовать в нём нечего.
   *
   * Выброшенный отсюда, он не попадал в атлас — а укладчик строки шагает пером
   * по ширинам ИЗ АТЛАСА и на незнакомом знаке не шагает вовсе (`gl/text.ts`).
   * Слова слипались: «Выбирайте дверь» выходило «Выбирайтедверь» на каждом
   * экране забега и на каждом языке. Инстанса пробел всё равно не тратит:
   * глиф без отпечатка отдаёт пустой прямоугольник атласа, и укладчик его
   * пропускает — платим одной ячейкой текстуры за то, чтобы текст читался.
   */
