...
export function drawSettlement(r: Renderer, s: SimState, w: number, h: number, fb: Feedback): void {
  ...
  for (let p = 0; p < s.playerCount; p++) {
    ...
    for (let i = 0; i < MAX_ACTIVE_BETS; i++) {
      ...
      /*
       * Метка единицы измерения: песочные часы — секунды, две точки
       * столбиком — проценты. Без неё «4» и «40» читаются одинаково.
       *
       * Раньше секунды метились ТЕМ ЖЕ кольцом, что и отсчёт паузы ниже
       * (строка ~925) — тот же словарь форм на одном экране одновременно, и
       * readability-заход iter-8 отметил риск спутать метку единицы с
       * отдельным таймером автопродолжения. Песочные часы (два треугольника
       * остриями друг к другу — тот же приём, что у бабочки Крупье)
       * однозначно про «время» и ни с чем на этом экране не совпадают.
       */
      const d = PALETTE.loss;
      if (time) {
        const hx = w / 2 + 285;
        b.push(Shape.Triangle, hx, y - 4, 7, 5, -Math.PI / 2, d.r, d.g, d.b, 0.9, 0, 0, 0, 0, 0);
        b.push(Shape.Triangle, hx, y + 4, 7, 5, Math.PI / 2, d.r, d.g, d.b, 0.9, 0, 0, 0, 0, 0);
      } else {
        for (const dy of [-7, 7]) {
