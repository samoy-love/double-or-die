...
export const SCREEN_BINDINGS: readonly ScreenBinding[] = [
  // Крестовина ← → здесь намеренно не занята: в бою она отдана поштучному
  // забору пари (доступность, UX §5), а на экранах тот же жест дают стик и
  // `A`/`D` — заводить ей второй смысл незачем.
...
export class InputSource {
  ...
  attach(canvas: HTMLCanvasElement): void {
    window.addEventListener('keydown', (e) => {
      ...
      //
      // Tab — исключение: это единственный способ геймпада/Steam Deck
      // дотянуться до плашки «доступно обновление» (`overlay.ts`, тач без
      // явного tabindex до неё не доходит). `preventDefault` здесь глушится
      // безусловно на КАЖДОЕ нажатие — если плашка сейчас показана, отдаём
  ...
  /**
   * Навязать схему ввода: только отладка и съёмка.
   *
   * Обычного пути сюда нет и не должно быть: схему меняет исключительно сам
   * ввод (см. комментарий у поля `scheme`), а у агента съёмки нет ни рук, ни
   * геймпада — подключённый пад схему не переключает намеренно. Поэтому кадры
   * с подсказками и глифами пада иначе не снять вовсе.
   *
   * Держится ровно до первого живого ввода: любое нажатие, тап или заметное
   * движение мыши перепишет значение обратно. Для съёмки это безвредно, при
   * ручной отладке — ожидаемо, а не дефект.
   */
  forceScheme(v: InputScheme): void {
    this.scheme = v;
  }
