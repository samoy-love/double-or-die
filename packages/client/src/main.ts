...
async function main(): Promise<void> {
   ...
   *
   * Старт — это нажатие «Играть», а не загрузка страницы. С появлением меню
   * разница перестала быть формальной: открытая и закрытая вкладка забегом не
   * является, а счётчик решает, показывать ли «Выбор режима» (GDD §23).
   */
  loop.onRunStart(() => profile.countRun());
