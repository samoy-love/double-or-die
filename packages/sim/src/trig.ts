 ...
 * Длина вектора.
 *
 * Квадраты считаются НЕ в Q16.16: квадрат диагонали арены — это миллионы, а
 * формат держит ±32767 в целой части. Поэтому величины переводятся в обычные
 * числа, где все промежуточные значения точны в пределах мантиссы, а результат
 * возвращается обратно. Детерминизм при этом сохраняется: деление и сложение
 * IEEE 754 задано стандартом однозначно, в отличие от Math.sin и Math.atan2.
 */
export function length(x: Fx, y: Fx): Fx {
  ...
  /*
   * Начальное приближение — степень двойки не ниже корня, как в sqrt().
   *
   * Раньше стартом служил сам d2, и Ньютон тратил по итерации на каждое
   * удвоение расстояния до ответа: с 4.85 млн (диагональ арены) ему нужно
   * около пятнадцати шагов, а их было двенадцать. Длина диагонали получалась
   * 2312 вместо 2203 — ошибка в пять процентов, растущая вместе с ареной.
   *
   * Ловилось это плохо: на векторах прицела и скорости, где длина около
   * единицы, старый код сходился точно, и всё выглядело исправным.
   */
  let r = 1;
  while (r * r < d2) r *= 2;

  // От такого старта Ньютон удваивает число верных знаков за шаг: восьми
  // хватает на весь диапазон формата с запасом.
  for (let i = 0; i < 8; i++) {
    const next = (r + d2 / r) / 2;
    if (next === r) break;
    r = next;
  ...
  /*
   * Насыщение вместо заворачивания.
   *
   * Длина, не помещающаяся в Q16.16, — это выход за контракт формата, и
   * честного ответа тут нет. Но `| 0` превращал такую длину в ОТРИЦАТЕЛЬНУЮ,
   * а normalize() делит на неё: вектор разворачивался в противоположную
   * сторону. Упереться в потолок хотя бы заметно, развернуться — нет.
   */
...
/**
 * Помещается ли вектор в радиус `r`. Проверка близости без извлечения корня.
 *
 * Существует отдельно от `length()` потому, что по тику таких проверок тысячи:
 * восемьсот снарядов против двухсот врагов — это перебор пар, и Ньютон в
 * каждой из них съел бы кадр целиком. Квадраты считаются в обычных числах —
 * в Q16.16 квадрат расстояния не помещается в принципе, — а детерминизм
 * сохраняется: деление на степень двойки, умножение и сложение IEEE 754
 * заданы стандартом однозначно.
 */
export function within(dx: Fx, dy: Fx, r: Fx): boolean {
  // Дешёвая отсечка по осям: подавляющее большинство пар отсеивается целым
  // сравнением, не дойдя до умножения.
