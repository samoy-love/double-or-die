...
export function spawnEnemy(s: SimState, type: EnemyType, x: Fx, y: Fx, novice = false): number {
  for (let i = 0; i < MAX_ENEMIES; i++) {
     ...
     * тот, что выходит первым на пустую арену первой волны. Отдельного слота
     * `Meta` под «уже помечен» не заводим (свободных не осталось): условие
     * «волна 1» и «арена ещё пуста» само по себе выполняется ровно однажды за
     * комнату, потому что вторая волна начинает уже с ненулевым `Wave`.
     */
    const mark =
