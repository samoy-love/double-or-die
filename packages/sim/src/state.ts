...
export const enum Meta {
  ...
  SpawnOff = 9,
  /**
   * Что Туз сейчас делает телом и до какого тика (GDD §17А).
   *
   ...
   * при переигровке оказался бы другим — и клип с зевающим Тузом перестал бы
   ...
   *
   * Оба числа нужны Тузу: по первому он понимает, что игрок третью комнату
  ...
  DeathStreak = 22,
  /**
   * Тик, на котором Туз уходит с арены. Ноль — уходить некому.
   *
...
/**
 * Чем Туз занят, пока игрок воюет.
 *
...
export enum AceGesture {
  ...
  Fidget = 4,
  /** Соскочил на 90% прогресса: молча показывает большой палец вниз. */
  ThumbsDown = 5,
