...
describe('бюджет угрозы по этажам', () => {
  /*
   * Комната 2, а не 1: у комнаты 1 этажа 1 есть скидка первого знакомства
   * (`WAVE.firstRoomPct`, см. тест ниже) — она разово ломает удвоение
   * по этажу для конкретно этой пары этаж/комната, и это ожидаемо, а не
   * повод чинить тест «раздваиванием» константы под исключение.
   */
  it('удваивается с каждым этажом', () => {
    const first = roomBudget(2, 1, 1);
    expect(roomBudget(2, 1, 2)).toBe(first * 2);
    expect(roomBudget(2, 1, 3)).toBe(first * 4);
  });
   ...
   * секунд боя». Комната 1 этажа 1 — со скидкой (см. тест ниже), поэтому
   * здесь она не участвует.
   */
  it('сходится с таблицей целевых длительностей', () => {
    expect(roomBudget(2, 1, 1)).toBe(306);
    expect(roomBudget(ROOMS_PER_FLOOR, 1, 1)).toBe(396);
    expect(roomBudget(2, 1, 2)).toBe(612);
    expect(roomBudget(2, 1, 3)).toBe(1224);
  });

  it('комната считается внутри этажа, а не сквозной по забегу', () => {
    // Вторая комната второго этажа обязана быть ровно вдвое тяжелее второй
    // комнаты первого — а не десятой, которой не существует. Не комната 1:
    // у неё своя скидка (см. ниже), не связанная с этим правилом.
    expect(roomBudget(2, 1, 2)).toBe(roomBudget(2, 1, 1) * 2);
  });

  /**
   * Скидка первого знакомства (`WAVE.firstRoomPct`, playtest: «куча врагов,
   * толпой давят») — 70% от того, что дала бы обычная формула, и только у
   * комнаты 1 этажа 1. Комната 1 любого другого этажа — забег уже идёт,
   * скидки нет.
   */
  it('комната 1 этажа 1 — со скидкой первого знакомства', () => {
    expect(roomBudget(1, 1, 1)).toBe(210);
