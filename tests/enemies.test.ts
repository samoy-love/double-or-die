...
describe('волны', () => {
  it('бюджет угрозы растёт по формуле из DIFFICULTY §4', () => {
    // Излом на комнате 5 (playtest 0.3.1, ре-бейзлайн): 2%/комнату до неё,
    // 8%/комнату после — не прямая, но комната 1 не видит разницы никогда.
    // Комната 1 этажа 1 (умолчание `floor` в `roomBudget`) — со скидкой
    // первого знакомства (`WAVE.firstRoomPct`, playtest: «куча врагов,
    // толпой давят»): 70% от того, что дала бы обычная формула.
    expect(roomBudget(1, 1)).toBe(210);
    expect(roomBudget(8, 1)).toBe(396);
    expect(roomBudget(1, 4)).toBe(714);
  });
