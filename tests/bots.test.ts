...
describe('ось навыка различима измеримо', () => {
   ...
   * по десяти забегам, а не один: разброс исходов — свойство игры про азарт,
   * и один забег ничего не доказывает ни в какую сторону. Замер на момент
   * написания: мастер живёт вдвое дольше, порог поставлен на +25%, чтобы
   * тест ловил исчезновение разницы, а не колебания баланса.
   */
  it('мастер выживает дольше новичка', () => {
    let novice = 0;
    let master = 0;
    for (let seed = 1; seed <= 10; seed++) {
      novice += survive('novice:stack', seed, 6000);
      master += survive('master:stack', seed, 6000);
    }
  ...
  it('`chips` подбирает больше фишек, чем тот же навык без погони за ними', () => {
    const earned = (name: BotName): number => {
      ...
      for (let seed = 1; seed <= 6; seed++) {
        ...
        for (let t = 0; t < 2400; t++) step(s, b.inputs(s));
        total += s.pChips[0];
      }
      return total;
    };
...
describe('наблюдение за забегом', () => {
  it('пари попадают в разбор по id, тиру и исходу', () => {
    ...
    // Стак играет «По-крупному» — это половина профиля, и её видно в разборе.
    expect(bets.every((x) => x.tier === 'go_big')).toBe(true);
