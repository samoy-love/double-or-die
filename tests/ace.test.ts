...
describe('Ставка Крупье: выплата', () => {
  ...
  it('проигранная уходит в «отдано Крупье»', () => {
    const s = fresh(400);
    const stake = shake(s);
    failBet(s, 0, 'no_dash');
...
describe('Ставка Крупье: кошелёк не уходит в минус', () => {
  it('проигрыш забирает только то, что есть', () => {
    const s = fresh(400);
    const stake = shake(s);
    expect(stake).toBeGreaterThan(0);

    // Между рукопожатием и провалом игрок разорился — Крупье в кредит не
...
describe('Ставка Крупье: частота', () => {
  ...
  it('больше одной ставки на игроке сразу не висит', () => {
    ...
    expect(aces).toBe(2);
    expect(() => checkInvariants(s)).toThrow(/Ставки Крупье сразу/);
  });
});
